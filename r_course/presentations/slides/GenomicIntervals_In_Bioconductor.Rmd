---
title: "Genomic Intervals In Bioconductor"
...
```
---
# Retreiving chromosome/contig names from a GenomicRanges.

We can retreive the chromosome names in a similar manner to **start()** and **end()** functions using the **seqnames()** function.
...
To add new contig we must update the seqlevels, like levels for factors but now for contigs in GRanges obbjects.
...
Similarly we can extract and set strand information using the **strand()** fucntion.
```{r atra,error=TRUE}
...
myGenomicIntervals <- GRanges(seqnames=c("chr1","chr1","chr2"), myIntervals,strand=c("+","+","+"))
strand(myGenomicIntervals)
```
---
...
To add additional metadata columns we can simply supply the desired column name and the values we want to assign as with data.frames.
...
We to retieve all metadata columns as a data frames we can use the **mcols()** function
...
The include functions to extend, narrow, merge and overlap genomic ranges.
---
# Adjusting genomic ranges postions.
...
by default the **resize()** function adjust the width of regions from the **start** of the region. We can control this behaviour by specifying the **fix** parameter as start, end or centre to speciy where we wish to resize from.
...
For negative strand intervals, when specifying **fix=start** it considers the start as the end position.
...
strand(myGenomicIntervals) <- c("+","-","+")
mergedGenomicIntervals <- reduce(myGenomicIntervals,ignore.strand=TRUE)
mergedGenomicIntervals
```
...
Some particularly useful functions/methods in GenomicRanges allows us to assess the relationship between two sets of GenomicRanges.

One of the most common is the **%over%** method which allows us to identify overlapping regions.

First we make two genomic ranges lists.

```{r ajfddfucfdjlsa,error=TRUE}
myGenomicIntervals1 <- GRanges(seqnames=c("chr1","chr1"),
                              ranges=IRanges(start=c(1,25),end=c(20,30)),
                              strand=c("+","+"))
myGenomicIntervals2 <- GRanges(seqnames=c("chr1","chr1"),
                              ranges=IRanges(start=c(22,100),end=c(27,130)),
                              strand=c("+","+"))
```
...
We can use this logical vector then to subset our **myGenomicIntervals1** set.
...
The **rtracklayer** package provide mechansisms to import and export from bed3/6 files using the **import.bed** and **export.bed** functions.

```{r revruujje,error=TRUE}
library(rtracklayer)
mySicerPeaks <- import.bed("../../Data/SicerPeaks.bed")
mySicerPeaks
```
---
# Importing and exporting to bed.
...
With GRanges objects we can specify the BSgenome and GRanges as regions to be extracted.
...
[Link_to_exercises](../../exercises/GenomicRanges_exercises.html)
