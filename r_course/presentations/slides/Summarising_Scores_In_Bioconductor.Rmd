---
title: "Summaris/zing Scores In Bioconductor"
...
* **In ChIP-seq** experiments we compare the number of DNA fragments mapping to a genomic locus after an IP for a specific protein or DNA/histone state versus the number of DNA fragments mapping to a genomic locus from a pool of genomic DNA.
...
Simple genomic regions such as peaks or TSS regions are often stored as **BED** files.

More complex, multi-part regions such as gene models are often stored as **GTF** or **GFF** files.
...
Here we see scores only on chromosome 12 around the genome we are looking at.
...
heartCoverageNorm <- coverage("Heart.bam")
mean(heartCoverageNorm[slc25A3])
```
...
* Assign reads overlapping multiple regions.
...
To count of non-overlapping regions, we can simply specify the path to our BAM file and a GRanges of regions to count over.
...
```{r a1312a}
allGeneCounts <- summarizeOverlaps(geneExons,c("Heart.bam","Liver.bam"))
```
...
```{r a22w}
myBam <- BamFileList(c("Heart.bam","Liver.bam"),yieldSize = 1000)
