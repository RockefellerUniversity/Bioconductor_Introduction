...
10. Generate density plots of summit heights for summits overlapping and not overlapping our extended TSS positions
...
ggplot(mySummitsDF,aes(x=mySummitsDF$score,fill=Overlap))+geom_density(alpha=0.5)+
  facet_wrap(~seqnames)+theme_bw()
```

11. Filter the Summits GRanges to the top 500 ranked by the GRanges score column.
```{r installBzsaaassS2}
orderSummits500 <- mySummits[order(mySummits$score,decreasing = TRUE)][1:500,]
```

12. Extend these top 500 Summits GRanges to 50bps around the centre of the GRanges intervals. Extract the sequences under the peaks and write to a file. 
